...
@Configuration
public class ClientArchWriter implements ItemWriter<sss_cdr_arch_client_stat> {
    ...
    public ClientArchWriter(sss_cdr_arch_client_stat_Repository sssCdrArchClientStatRepository) {
        sss_cdr_arch_client_stat_Repository = sssCdrArchClientStatRepository;
    }

    @Override
    public void write(Chunk<? extends sss_cdr_arch_client_stat> chunk) throws Exception {
        for (sss_cdr_arch_client_stat client : chunk) {
            boolean exists = sss_cdr_arch_client_stat_Repository.findById_lotAndDateExtractionAndId_client(
